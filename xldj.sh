...
# 生成节点所需的随机值，不依赖 sing-box generate 的参数格式。
# sing-box 仅继续负责无法由通用工具替代的 Reality 密钥对生成。
_sb_rand_hex() {
    local bytes="${1:-16}"
    local value=""
    [[ "$bytes" =~ ^[0-9]+$ ]] || return 1
    if command -v openssl >/dev/null 2>&1; then
        value=$(openssl rand -hex "$bytes" 2>/dev/null) || return 1
    ...
    if [ -x "${SINGBOX_BIN:-/usr/local/bin/sing-box}" ]; then
        "${SINGBOX_BIN:-/usr/local/bin/sing-box}" generate rand "$bytes" --hex 2>/dev/null || \
            "${SINGBOX_BIN:-/usr/local/bin/sing-box}" generate rand --hex "$bytes" 2>/dev/null
        return $?
    fi
    return 1
}
...
_sb_rand_base64() {
    ...
    [[ "$bytes" =~ ^[0-9]+$ ]] || return 1
    if command -v openssl >/dev/null 2>&1; then
        value=$(openssl rand -base64 "$bytes" 2>/dev/null) || return 1
        [ -n "$value" ] || return 1
        printf '%s\n' "$value"
        return 0
    fi
    if [ -x "${SINGBOX_BIN:-/usr/local/bin/sing-box}" ]; then
        "${SINGBOX_BIN:-/usr/local/bin/sing-box}" generate rand "$bytes" --base64 2>/dev/null || \
            "${SINGBOX_BIN:-/usr/local/bin/sing-box}" generate rand --base64 "$bytes" 2>/dev/null
        return $?
    fi
    return 1
}
...
_sb_uuid() {
    ...
    hex="$(_sb_rand_hex 16)" || return 1
    [ "${#hex}" -ge 32 ] || return 1
    local clock_seq
    printf -v clock_seq '%02x' $((0x${hex:16:2} & 0x3f | 0x80))
    printf '%s-%s-%s-%s%s-%s\n' \
        "${hex:0:8}" "${hex:8:4}" "4${hex:13:3}" "$clock_seq" "${hex:18:2}" "${hex:20:12}"
}
...
_check_port_in_config() {
    local port=$1
    local config_path
    for config_path in "$CONFIG_FILE" "${SINGBOX_DIR}/relay.json"; do
        [ -f "$config_path" ] || continue
        if jq -e ".inbounds[] | select(.listen_port == ($port|tonumber))" "$config_path" >/dev/null 2>&1; then
            return 0
        fi
    done
    return 1
...
_install_sing_box() {
    ...
    local candidate_bin=""
    local backup_bin=""
    local update_id="$(date +%s 2>/dev/null || echo 0).$$"
    case $arch in
    ...
    # 先在临时路径验证候选内核，避免升级时直接覆盖可用版本。
    candidate_bin="${temp_dir}/sing-box.candidate"
    if ! cp "$extracted_bin" "$candidate_bin" || ! chmod +x "$candidate_bin"; then
        _error "准备 sing-box 候选二进制失败。"
        return 1
    fi
    if ! "$candidate_bin" version >/dev/null 2>&1; then
        _error "候选 sing-box 二进制无法执行，已取消更新。"
        return 1
    fi
    if [ -s "$CONFIG_FILE" ] && ! "$candidate_bin" check -c "$CONFIG_FILE" >/dev/null 2>&1; then
        _error "候选 sing-box 无法通过主配置检查，已取消更新。"
        _warn "现有内核保持不变；请先执行主菜单中的配置检查并修复配置。"
        return 1
    fi
    if [ -s "${SINGBOX_DIR}/relay.json" ] && ! "$candidate_bin" check -c "${SINGBOX_DIR}/relay.json" >/dev/null 2>&1; then
        _error "候选 sing-box 无法通过 relay.json 检查，已取消更新。"
        _warn "现有内核保持不变；请检查中转配置文件。"
        return 1
    fi
    if [ -s "$CONFIG_FILE" ] && [ -s "${SINGBOX_DIR}/relay.json" ] && \
       ! "$candidate_bin" check -c "$CONFIG_FILE" -c "${SINGBOX_DIR}/relay.json" >/dev/null 2>&1; then
        _error "候选 sing-box 无法通过主配置与 relay.json 合并检查，已取消更新。"
        _warn "现有内核保持不变；请检查重复 tag、端口或路由引用。"
        return 1
    fi
    ...
    if [ -f "$SINGBOX_BIN" ]; then
        backup_bin="${SINGBOX_BIN}.previous.${update_id}"
        if ! cp -p "$SINGBOX_BIN" "$backup_bin"; then
            _error "备份现有 sing-box 二进制失败: $backup_bin"
            return 1
        fi
        export SB_UPDATE_BACKUP_BIN="$backup_bin"
    else
        export SB_UPDATE_BACKUP_BIN=""
    fi
    if ! mv -f "$candidate_bin" "$SINGBOX_BIN"; then
        _error "安装 sing-box 二进制文件失败: $SINGBOX_BIN"
        _error "临时目录保留: $temp_dir"
        _rollback_singbox_update || true
        return 1
    fi
...
_rollback_singbox_update() {
    local backup_bin="${SB_UPDATE_BACKUP_BIN:-}"
    [ -n "$backup_bin" ] && [ -f "$backup_bin" ] || return 1
    _warn "正在恢复升级前的 sing-box 内核..."
    if ! cp -p "$backup_bin" "$SINGBOX_BIN"; then
        _error "恢复 sing-box 内核失败，请手动复制: $backup_bin"
        return 1
    fi
...
_check_and_fix_dns() {
    ...
    # sing-box 1.12+ DNS server 使用 type=local；旧版 address=local 会在 1.14 移除。
    ...
    if [ "$has_dns" == "false" ] || [ "$has_auto_detect" == "true" ] || \
       ! jq -e 'try (
        ...
        _warn "检测到配置文件 DNS/路由与当前模板不一致，正在自动迁移到 sing-box 1.12+ 格式..."
        ...
        if [ $? -eq 0 ] && [ -s "$tmp_file" ]; then
            ...
            _success "DNS 与路由参数热修复完成：dns-local / type=local / ipv4_only。"
            needs_restart=true
        else
...
_add_hysteria2() {
    ...
    if [ "$BATCH_MODE" = "true" ]; then
        ...
        # 批量模式 double check
        [ -z "$node_ip" ] && node_ip="$server_ip"
        [ "$BATCH_HY2_OBFS" != "none" ] && obfs_password=$(_sb_rand_hex 16)
    ...
    if [ "$BATCH_MODE" = "true" ]; then
        ...
        read -p "请输入密码 (默认随机): " password; password=${password:-$(_sb_rand_hex 16)}
        read -p "是否开启 QUIC 流量混淆 (salamander)? (y/N): " h_choice
        if [[ "$h_choice" == "y" ]]; then
            obfs_password=$(_sb_rand_hex 16)
        fi
...
_add_tuic() {
    ...
    local uuid=$(_sb_uuid); local password=$(_sb_rand_hex 16)
...
_add_shadowsocks_menu() {
    ...
    if [ "$BATCH_MODE" = "true" ]; then
        port="$BATCH_PORT"
        if ! _validate_listen_port "$port"; then
            _error "批量创建 SS 失败：端口 ${port} 无效。"
            return 1
        fi
        _check_port_conflict "$port" "tcp" "true" && {
            _error "批量创建 SS 失败：端口 ${port} 已被占用。"
            return 1
        }
    else
        read -p "请输入服务器IP地址 (默认: ${server_ip}): " custom_ip
    ...
    if [ "$use_shadowtls" == "true" ]; then
        shadowtls_password=$(_sb_rand_hex 16)
        if [ "$BATCH_MODE" = "true" ]; then
            shadowtls_sni="${BATCH_SNI:-$(_get_random_sni)}"
        else
...
_add_socks() {
    ...
    if [ "$BATCH_MODE" = "true" ]; then
        ...
        read -p "请输入用户名 (默认随机): " username; username=${username:-$(_sb_rand_hex 8)}
        read -p "请输入密码 (默认随机): " password; password=${password:-$(_sb_rand_hex 16)}
    fi
...
_check_config() {
    ...
    if [ -s "${SINGBOX_DIR}/relay.json" ]; then
        result=$(${SINGBOX_BIN} check -c "${SINGBOX_DIR}/relay.json" 2>&1)
        if [[ $? -eq 0 ]]; then
            _success "中转配置 (${SINGBOX_DIR}/relay.json) 格式正确。"
        else
            _error "中转配置检查失败:"
            echo "$result"
            return 1
...
_do_update_singbox() {
    ...
    local config_backup="${CONFIG_FILE}.preupdate"
    local relay_config="${SINGBOX_DIR}/relay.json"
    local relay_backup="${relay_config}.preupdate"
    local install_rc=0

    [ -s "$CONFIG_FILE" ] && cp -p "$CONFIG_FILE" "$config_backup" 2>/dev/null || true
    [ -s "$relay_config" ] && cp -p "$relay_config" "$relay_backup" 2>/dev/null || true

    # 先迁移旧版 DNS server，再让候选内核执行 check；否则 1.12+ 会在
    # 更新阶段就拒绝旧的 address=local 配置，导致无法完成升级。
    [ -s "$CONFIG_FILE" ] && _check_and_fix_dns || true

    _install_sing_box || install_rc=$?
    ...
    if [ "$install_rc" -eq 0 ]; then
        ...
        if _manage_service "restart"; then
            _success "[主] 服务已就绪。"
            rm -f "$config_backup" "$relay_backup" "${SB_UPDATE_BACKUP_BIN:-}"
            export SB_UPDATE_BACKUP_BIN=""
        else
            _error "新 sing-box 内核启动失败，准备自动回滚。"
            _rollback_singbox_update || true
            if [ -s "$config_backup" ]; then cp -p "$config_backup" "$CONFIG_FILE"; fi
            if [ -s "$relay_backup" ]; then cp -p "$relay_backup" "$relay_config"; fi
            _create_service_files
            if _manage_service "restart"; then
                _warn "已恢复升级前内核和配置，服务已重新启动。"
            else
                _error "回滚后服务仍未启动，请检查 sing-box 日志。"
            fi
            rm -f "$config_backup" "$relay_backup"
            return 1
        fi
    else
...
_advanced_features() {
    ...
    # 17 进阶中转脚本会对主配置和 relay.json 做组合校验。
    # 在进入子脚本前先迁移旧版 DNS server，避免 sing-box 1.12+ 将
    # address=local 等 legacy 格式判定为错误（1.14 将彻底移除）。
    if [ -s "$CONFIG_FILE" ]; then
...
_batch_create_nodes() {
    local input_str="$1"
    if [ -z "$input_str" ]; then
        _info "请输入协议编号 (空格或逗号分隔，如: 1,2,3,4)"
        _info "1=VLESS (Vision+REALITY), 2=AnyTLS, 3=Hysteria2, 4=Shadowsocks"
    ...
    for pid in $proto_ids; do
        if ! [[ "$pid" =~ ^(1|2|3|4)$ ]]; then
            _error "协议 ID $pid 无效。批量创建支持：1=VLESS (Vision+REALITY)、2=AnyTLS、3=Hysteria2、4=Shadowsocks。"
            return 1
        fi
        ((proto_count++))
        [[ "$pid" =~ ^(1|2|3)$ ]] && has_sni_req=true
        [[ "$pid" == "3" ]] && has_hy2=true
        [[ "$pid" == "4" ]] && has_ss=true && ((ss_occurences++))
    done
    ...
    local ss_variant="1"
    if [ "$has_ss" = true ]; then
        read -p "请选择 SS 加密方式 [1=aes-256-gcm, 2=chacha20, 3=SS-2022, 4=SS-2022+Padding, 5=SS-2022+ShadowTLS] (默认: 1): " ss_variant_input
        ss_variant="${ss_variant_input:-1}"
        if ! [[ "$ss_variant" =~ ^[1-5]$ ]]; then
            _error "无效的 SS 加密方式。"
            return 1
        fi
        export BATCH_SS_VARIANT="$ss_variant"
    fi
    ...
    while true; do
        ...
        for candidate_port in "${current_p_list[@]}"; do
            if ! _validate_listen_port "$candidate_port"; then
                invalid_port=true
                break
            fi
        done
        if [ "$invalid_port" = true ]; then
            _error "端口必须是 1-65535 的整数，请重新输入。"
        elif [ ${#current_p_list[@]} -lt $proto_count ]; then
            _error "输入端口数量不足（仅 ${#current_p_list[@]} 个），请重新输入。"
        else
            local duplicate_port=""
            for ((port_idx=0; port_idx<${#current_p_list[@]}; port_idx++)); do
                for ((seen_idx=0; seen_idx<port_idx; seen_idx++)); do
                    if [ "${current_p_list[$port_idx]}" = "${current_p_list[$seen_idx]}" ]; then
                        duplicate_port="${current_p_list[$port_idx]}"
                        break 2
                    fi
                done
            done
            if [ -n "$duplicate_port" ]; then
                _error "端口 ${duplicate_port} 在本次批量规划中重复，请重新输入。"
            else
                local occupied_ss_port=""
                local planned_idx=0
                for planned_pid in $proto_ids; do
                    if [ "$planned_pid" = "4" ] && _check_port_conflict "${current_p_list[$planned_idx]}" "tcp" "true"; then
                        occupied_ss_port="${current_p_list[$planned_idx]}"
                        break
                    fi
                    ((planned_idx++))
                done
                if [ -n "$occupied_ss_port" ]; then
                    _error "SS 端口 ${occupied_ss_port} 已被占用，请重新输入。"
                else
...
_show_add_node_menu() {
    ...
    echo -e "    ${GREEN}[4]${NC} Shadowsocks (SS)"
    echo ""
    ...
    case $choice in
        ...
        4) _add_shadowsocks_menu; action_result=$? ;;
        10) _batch_create_nodes; return ;;
        0) return ;;
        *) _error "无效输入。当前版本支持创建 VLESS (Vision+REALITY)、AnyTLS、Hysteria2、Shadowsocks。" ;;
    esac
